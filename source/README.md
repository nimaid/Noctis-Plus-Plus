...
Alternatively, you can download the portable version and extract it's `bin` folder to the main `Noctis-Plus-Plus` folder.
### \*nix
...
2. Make a new folder in the main `Noctis-Plus-Plus` folder named `bc.31`.
...
1. Double-click `source\Build.bat` (Windows) or `source\Build.bash` (\*nix) to run the build script.
    ...
    - If everything is downloaded correctly, it will open DOSBox-X for compilation.
        ...
        - If the build succeeds, it will prompt you to press a key to run the newly compiled Noctis++.

The compiled binary is automatically copied over to the `modules` folder inside the main `Noctis-Plus-Plus` folder.
